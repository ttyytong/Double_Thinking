...
def forward_step(data_iterator, model, args, timers):
    ...
    # Append the loss value to the list at every 100 steps
...
def create_dataset_function(image_processor, text_processor, path, args):
    dataset = ItemDataset(image_processor=image_processor,
...
def plot_losses(train_losses):
    ...
    steps = range(0, len(train_losses)*100, 100)
    smoothed_losses = np.convolve(train_losses, np.ones(5)/5, mode='valid')  # Apply smoothing
    smoothed_steps = range(0, len(smoothed_losses)*100, 100)  # Adjust steps for the smoothed values
    plt.plot(smoothed_steps, smoothed_losses, label="Training Loss")
    plt.xlabel("Steps")
    plt.ylabel("Loss")
...
if __name__ == "__main__":
    args = get_args()
    args.finetune = True
    model, args = FineTuneTrainCogVLMModel.from_pretrained(model_args=args, pretrained_model_name_or_path=args.load)
    args.enable_2d_positional_encoding = True
    tokenizer = get_tokenizer(args)
    image_processor = get_image_processor(args)
    data_collator = partial(data_collator)
    create_dataset_function = partial(create_dataset_function, image_processor, llama2_text_processor, args.train_data_path, args)
    training_main(args, model_cls=FineTuneTrainCogVLMModel,
