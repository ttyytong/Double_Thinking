...
def forward_step_eval(data_iterator, model, args, timers):
    def compute_metrics(eval_preds):
        ...
        # 打开文件进行写入
        with open("/ssd0/tyt/CogVLM/evaluation_results.txt", "a") as f:
            for pred, label in zip(decoded_preds, decoded_labels):
                if args.rank == 0:
                    output_str = f'pred: {pred}, label: {label}\n'
...
if __name__ == '__main__':
    ...
    py_parser.add_argument("--from_pretrained", type=str, default="/ssd0/tyt/CogVLM/checkpoints/merged_model_490", help='pretrained ckpt')
